...
# To store results for all years (optional, for later use)
yearwise_metrics <- list()
yearwise_confusion <- list()

for (yr in years) {
  cat("==========================================\n")
  cat("Running models for year:", yr, "\n")
  cat("==========================================\n")
  ...
  data_path <- paste0("C:/Users/ady27/Desktop/combined2/", yr, "OME.dta")
  ...
  # --- 7. XGBoost ---
  xgb_model <- xgboost(data = over_matrix, label = as.numeric(over$TEAyyOPP_factor) - 1,
  ...
  h2o_gbm_model <- h2o.gbm(x = x, y = y, training_frame = train_h2o,
                           ntrees = 100, learn_rate = 0.1, max_depth = 6, seed = 1234)
  ...
  # ==========================================
  # 5. RESULTS AND PLOTTING (PER YEAR)
  # ==========================================
...
#  legend("bottomright", legend = final_metrics$Model, col = colors_list, lwd = 2, cex = 0.6)
  ...
  # Print Final Outputs for this year
  cat("\nFinal Metrics (Positive Class: YES) - Year", yr, ":\n")
  print(final_metrics)
...
# After loop, you have:
# yearwise_metrics[["2015"]], yearwise_metrics[["2016"]], ..., yearwise_metrics[["2020"]]
# and similarly for yearwise_confusion
...
# Save file
saveWorkbook(wb, 
             file = "C:/Users/ady27/Desktop/OMEWorldYearWise_2015_2020_Results.xlsx",
             overwrite = TRUE)

cat("\nExcel Export Completed: OMEWorldYearWise_2015_2020_Results.xlsx\n")
